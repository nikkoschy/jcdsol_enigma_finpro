...
F2 score : Chosen because of the need to achieve for higher recall while still being able to balance both recall and precision. This model needs to be able to gather more customer leads who will be likely to subscribe to the term deposit while at the same time needing to keep the contact number of non-deposit customer low. Using F-2 score as metrics evaluation allows us to give more weight to the recall so that the model could reach more positive customer leads by making sure it has considerable precision, subsequently lowering the total cost generated by the False Negatives and providing more leniencies for False Positive predictions. 
